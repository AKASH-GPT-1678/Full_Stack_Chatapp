...
const MainSideDisplay = () => {
  ...
  const addNewUser = async () => {
    try {
      const response = await axios.post(
        `${endpoint}/api/addcontactchatter`,
        { contactId: addChat.trim() },
        {
          headers: {
  ...
  const makeRequest = async (requestid) => {
    ...
    try {
      const response = await axios.put(
        `${endpoint}/api/acceptrequestchatter`,
        { requestId: requestid },
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const activeContact = myContacts.filter(
  ...
  const filterRequest = requests.filter(
    (request) => request.ownerId !== userId,
  );
  const activeGroup = [groups.find((group) => group.id === activeChat)];
  const commonProfile =
    "https://res.cloudinary.com/dffepahvl/image/upload/v1754295798/pwoveg1fjurga2kudwk4.png";
  ...
  return (
    <div className="flex flex-row w-full bg-gray-200 md:p-8 p-4 gap-8 ">
      <div className="flex flex-col w-full min-w-[350px] md:min-w-[400px] md:max-w-[500px] p-3 min-h-screen bg-white rounded-2xl shadow-2xl">
        <div className="flex flex-row justify-between max-w-[400px] xl:max-w-[500px] items-center">
          <div className="p-3 flex flex-row gap-2">
            <img
              src={profileImage || commonProfile}
              alt="profileimage"
          ...
          <div className="flex flex-row gap-3  p- relative">
            ...
            <div className="relative">
              {filterRequest.length > 0 && (
                <div className="bg-red-500 h-3 w-3 rounded-full  absolute top-0 right-2"></div>
              )}
              <BsThreeDotsVertical
                size={30}
            ...
            {showOptions ? (
              <div
                className="absolute top-10 right-0 z-40 bg-white p-4 min-w-[150px] lg:min-w-[200px] border-t-2 rounded-xl shadow-2xl  space-y-4"
                ref={divRef}
              >
                <p className="font-semibold" onClick={() => setNewUser(true)}>
                  New Chat
                </p>
                <p
                ...
                <p
                  className="font-semibold cursor-pointer flex flex-row gap-2 items-center"
        ...
        <div>
          <div className="p-4 bg-gray-200 w-full rounded-3xl flex flex-row items-center min-h-[50px] justify-evenly">
            <p
              className={`${
                activeType === "default"
                  ? "px-6 rounded-2xl text-blue-500 bg-white py-2"
                  : ""
            ...
            <p
              className={`${
                activeType === "groups"
                  ? " rounded-2xl text-blue-500 bg-white "
                  : ""
              } cursor-pointer font-bold text-gray-500  px-4 md:px-8 py-2`}
          ...
          <div>
            <input
              type="text"
              className="w-full p-3 border border-gray-400 mt-4  rounded-2xl"
              placeholder="Search"
            />
          </div>
            ...
            <div className="mt-3 h-full relative">
              {contactsArray.length > 0 &&
                myContacts.map((contact, index) => (
                  <div
                    key={`contact-${index}`}
                    className="flex flex-row min-h-[50px] relative  items-center cursor-pointer p-2  rounded-2xl hover:bg-gray-50"
                    ...
                    <img
                      src={
                        contact.profileImage
                          ? contact.profileImage
                          : commonProfile.toString()
                      }
                      alt={contact.username}
                      className="rounded-full object-cover h-[60px] w-[70px] border border-gray-400"
                    />
                    ...
                    <div className="flex flex-col ml-3 w-full">
                      <div className="flex flex-row justify-between">
                        ...
                        {newGroup ? (
                          <input
                            type="checkbox"
                            className="flex flex-col accent-green-600  cursor-pointer self-center-safe p-2 h-6 w-12"
                            onChange={(e) =>
                              handleCheckboxChange(e, contact.id)
                            }
                          />
                        ) : (
                      ...
                      <p className="mt-2">Hey how are you doing</p>
                    </div>
                  </div>
                ))}
              ...
              {groups.length > 0 &&
                groups.map((contact, index) => (
                  <div
                    key={`group-${index}`}
                    className="flex flex-row min-h-[50px] relative  items-center cursor-pointer p-2  rounded-2xl hover:bg-gray-50"
                    onClick={() => handleGroupPage(contact.id)}
                  >
                    <img
                      src={
                        contact.groupProfile
                          ? contact.groupProfile
                          : commonProfile.toString()
                      }
                      alt={contact.username}
              ...
              {items.length > 0 && (
                <IoArrowForwardCircle
                  size={50}
                  className="ml-auto cursor-pointer absolute  bottom-12 right-3.5"
                  fill="green "
                  onClick={() => handlecreateGroup(items)}
                />
              )}
            </div>
          )}
      ...
      <div className="w-full hidden sm:block">
        {groupChat ? (
          <UserChats
            username={activeGroup.length > 0 ? activeGroup[0].groupName : ""}
            chatId={activeGroup.length > 0 ? activeGroup[0].id : ""}
            type={"group"}
          />
        ) : (
          <UserChats
            username={activeContact.length > 0 ? activeContact[0].username : ""}
            chatId={activeContact.length > 0 ? activeContact[0].id : ""}
            type={"chat"}
          />
        )}
      </div>
   ...
   <NewUserModal setAddChat={setAddChat} setNewUser={setNewUser} addNewUser={addNewUser} />
      )}
    </div>
  );
};
